...
base_repos=$(find -L /git/ \( "${GITDIRS[@]}" -o -name logs -o -path /git/projects -o -path /git/toolchain -o -path /git/users -o -path /git/external -o -path /git/managed_builds -o -path /git/release \) -prune -o -type d -print )
...
echo "section=5-Release -----------------------------"
release_repos=$(find -L /git/release \( "${GITDIRS[@]}" -o -name '*-restricted' \) -prune -o -type d -print )
print_gitdir "$release_repos"
